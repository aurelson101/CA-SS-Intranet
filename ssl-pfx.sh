...
if [ ! -d "$CA_NAME" ]; then
    echo "CA directory not found. Make sure you have first run the xxca.sh script."
    exit 1
fi
...
# Check if the domain certificate files exist
if [ ! -f "${CA_NAME}/domains/${DOMAIN}/${DOMAIN}.crt" ] || [ ! -f "${CA_NAME}/domains/${DOMAIN}/${DOMAIN}.key" ]; then
    echo "Domain certificate files not found. Make sure you have first run the xxca.sh script for this domain."
    exit 1
fi
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "2. Importing into SQL Server:"
    ...
    echo ""
    echo "Note: Ensure you have the necessary permissions to import certificates and modify SQL Server configurations."
else
